...
/**
 * One decimal percent for Judge accuracy; falls back to correct/total when API omits `accuracy`.
 * @param {{ total_settled: number, correct: number, accuracy: number | null }} stats
 * @returns {string}
 */
function formatJudgeAccuracyPct(stats) {
  if (stats.accuracy != null && Number.isFinite(stats.accuracy)) {
    return `${Math.round(stats.accuracy * 1000) / 10}%`;
  }
  const total = stats.total_settled;
  if (total > 0) {
    const derived = stats.correct / total;
    if (Number.isFinite(derived)) {
      return `${Math.round(derived * 1000) / 10}%`;
    }
  }
  return "—";
}
 ...
 * @param {'intel'|'debate'|'judge'|'over'|'misc'} [groqRoute] forwarded to the proxy for Groq model + cap selection (ignored by Anthropic after strip)
 */
...
function syncAllIntelRefreshButtons() {
  for (const a of AGENTS) {
    syncIntelRefreshButtonForAgent(a.id);
  }
}
...
async function runWarRoom() {
  ...
  // Skip entirely if every agent had empty evidence (already filled with INTEL_FALLBACK above).
  const agentsNeedingLLM = AGENTS.filter((a) => evidenceByAgent[a.id]);
